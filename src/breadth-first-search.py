...
# Function to perform BFS and capture graph states
def bfs_capture_states(G, start_node):
    ...
    while queue:
        node = queue.popleft()
        visited.add(node)
        states.append(([node], list(visited), list(queue)))
        for neighbor in G.neighbors(node):
            if neighbor not in visited and neighbor not in queue:
                queue.append(neighbor)
        states.append(([node], list(visited), list(queue)))

    states.append(([], list(visited), list(queue)))
